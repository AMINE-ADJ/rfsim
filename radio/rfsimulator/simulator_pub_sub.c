...
static void allocCirBuf(rfsimulator_state_t *bridge, int sock)
{
  ...
  // Set the receive buffer size for the subscriber socket (if needed).
  ...
  if (bridge->channelmod > 0) {
    ...
    char *modelname = (bridge->typeStamp == SIMU_ROLE_SERVER) ? "rfsimu_channel_ue0" : "rfsimu_channel_enB0";
...
static void rfsimulator_readconfig(rfsimulator_state_t *rfsimulator)
{
  ...
  if (strncasecmp(rfsimulator->ip, "enb", 3) == 0 || strncasecmp(rfsimulator->ip, "server", 3) == 0)
    rfsimulator->typeStamp = SIMU_ROLE_SERVER;
  else
...
static int startServer(openair0_device *device)
{
  ...
  // ev.events = EPOLLIN;
  // ev.data.fd = t->listen_sock; // to check for a connection request
  // AssertFatal(epoll_ctl(t->epollfd, EPOLL_CTL_ADD, t->listen_sock, &ev) != -1,
  //             ""); // add the event to the epoll instance (listen sock will be monitored to check when data is ready to be read)
  // return 0;
  if (!t->context) {
...
static int startClient(openair0_device *device)
{
  ...
  // t->context = zmq_ctx_new();
  // AssertFatal(t->context != NULL, "Failed to create ZeroMQ context");
  // LOG_I("Device context created")

  // t->pub_sock = zmq_socket(t->context, ZMQ_PUB);
  // AssertFatal(t->pub_sock != NULL, "Failed to create Pub socket");
  // LOG_I("Pub socket created")

  // char pub_endpoint[256];
  // snprintf(pub_endpoint, sizeof(pub_endpoint), "tcp://%s:%d",t->brokerip, t->xsubport);

  // int rc = zmq_connect(t->pub_sock, pub_endpoint);
  // AssertFatal(rc == 0, "Failed to connect publisher socket");
  // LOG_I(HW,"Publisher connected to tcp://localhost:%d\n", t->xsubport);

  // t->sub_sock = zmq_socket(t->context, ZMQ_SUB);
  // AssertFatal(t->sub_sock != NULL, "Failed to create Sub socket");
  // LOG_I("Sub socket created")

  // char sub_endpoint[256];
  // snprintf(sub_endpoint, sizeof(sub_endpoint), "tcp://%s:%d",t->brokerip, t->xpubport);

  // rc = zmq_connect(t->sub_sock, sub_endpoint);
  // AssertFatal(rc >= 0, "Cannot connect to sub_endpoint");
  // LOG_I(HW,"Subscriber connected to tcp://localhost:%d\n", t->xpubport);

  // rc = zmq_setsockopt(subscriber, ZMQ_SUBSCRIBE, "downlink", strlen("downlink"));
  // AssertFatal(rc >= 0, "Device cannot subscribe to donwlink");

  // LOG_I("Device subscribed to downlink topic")
  ...
  // allocCirBuf(t, sock);// alloc the cirbuf from the client end
  // return 0;
...
// it is called internal because it is done at this level only
                                      ...
                                      bool alreadyLocked)
{
  ...
  for (int i = 0; i < FD_SETSIZE; i++) {
    ...
    if (b->conn_sock >= 0) {
      ...
      fullwrite(b->conn_sock, &header, sizeof(header), t); // writing the header
      ...
      for (int a = 0; a < nbAnt; a++) {
        ...
        for (int s = 0; s < nsamps; s++)
          tmpSamples[s][a] = in[s]; // s row, a column
      }
  ...
  if (t->lastWroteTS != 0 && fabs((double)t->lastWroteTS - timestamp) > (double)CirSize) // qsr why is this
    LOG_E(HW, "Discontinuous TX gap too large Tx:%lu, %lu\n", t->lastWroteTS, timestamp);

  if (t->lastWroteTS > timestamp + nsamps) // qst why is this
    LOG_E(HW, "Not supported to send Tx out of order (same in USRP) %lu, %lu\n", t->lastWroteTS, timestamp);

  t->lastWroteTS = timestamp + nsamps; // for the first time timestamp is lastwrote-1
...
static bool flushInput(rfsimulator_state_t *t, int timeout, int nsamps_for_initial)
{
  ...
  for (int nbEv = 0; nbEv < nfds; ++nbEv) {
    ...
    if (events[nbEv].events & EPOLLIN && fd == t->listen_sock) { // first connection
      int conn_sock;
      AssertFatal((conn_sock = accept(t->listen_sock, NULL, NULL)) != -1, ""); // qst starts looping if no ue connected?
      setblocking(conn_sock, notBlocking);
      allocCirBuf(t, conn_sock); // alloc the cirbuf from the server end
      ...
      rfsimulator_write_internal(t,
                                 t->lastWroteTS > 1 ? t->lastWroteTS - 1 : 0, // qst why -1
                                 samplesVoid,
                                 1,
                                 t->tx_num_channels,
                                 1,
                                 false); // qst is the nbant the same as tx_num...
    } else {
      if (events[nbEv].events & (EPOLLHUP | EPOLLERR | EPOLLRDHUP)) {
        socketError(t, fd);
        continue;
      } // other data then the connection request
      ...
      if (b->headerMode == true && b->remainToTransfer == 0) {
        AssertFatal((t->typeStamp == UE_MAGICDL && b->th.magic == SIMU_ROLE_SERVER)
                        || (t->typeStamp == SIMU_ROLE_SERVER && b->th.magic == UE_MAGICDL),
...
// qst why timestamp types are not the same, uint64 vs in64
...
__attribute__((__visibility__("default"))) int device_init(openair0_device *device, openair0_config_t *openair0_cfg)
{
  ...
  LOG_I(HW,
        ...
        rfsimulator->typeStamp == SIMU_ROLE_SERVER ? "server waiting opposite rfsimulators to connect"
                                              : "client: will connect to a rfsimulator server side");
  device->trx_start_func = rfsimulator->typeStamp == SIMU_ROLE_SERVER ? startServer : startClient;
  ...
  /* let's pretend to be a b2x0 */ // qsr what is this
  ...
  // AssertFatal((rfsimulator->epollfd = epoll_create1(0)) != -1,""); not necessary maybe
